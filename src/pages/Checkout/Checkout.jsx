...
function Checkout() {
  ...
  const handleSubmit = () => {
    const options = {
      ...
      key_secret: "11yI0aQAt8J8JmS52B5O7w8",
      amount: total * 100,
      currency: "INR",
      name: "Organic Mart",
      description: "Payment for Organic Mart Order",
  ...
  const saveAddress = () => {
    const { name, mobile, email, state, city, pincode, completeAddress } =
      address;
    ...
    if (editId) {
      updatedAddresses = savedAddresses.map((item) =>
        item.id === editId ? { ...address, id: editId } : item
      );
      setEditId(null);
    } else {
      updatedAddresses = [
    ...
    alert(
      editId ? "Address Updated Successfully" : "Address Saved Successfully"
    );
        ...
        <div className="checkout-layout">
          ...
          <div className="checkout-right">
            <div className="order-summary">
              ...
              {cart.length === 0 ? (
                <p className="empty-cart-msg">No Products Added</p>
              ) : (
                <>
                  {cart.map((item) => (
